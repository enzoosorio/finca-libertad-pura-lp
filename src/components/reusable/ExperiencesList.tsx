...
export const ExperiencesList = ({ experience, isLeft, experienceSelected, setExperienceSelected }: ExperiencesListProps) => {
  ...
  return (
    <div className={`absolute container-image-experience ${isLeft ? "top-40 left-[4%] container-images-experiences-left" : "top-40 right-[4%] left-auto container-images-experiences-right"}  flex flex-col items-center justify-around gap-12`}>
                ...
                key={index} className="group relative w-72 h-48 cursor-pointer aspect-video md:w-96 md:h-96 lg:w-80 lg:h-80 xl:w-96 xl:h-96 2xl:w-[450px] 2xl:h-[350px] overflow-hidden rounded-xs shadow-lg">
                    ...
                    <div className={`absolute inset-0 transition-all duration-300 z-20 ${
  experienceSelected === value 
    ...
    : 'bg-black/10 group-hover:bg-black/40' // No seleccionado: hover normal
                    ...
                    <p className={`absolute bottom-12 left-1/2 -translate-x-1/2 text-xl transition-opacity 
