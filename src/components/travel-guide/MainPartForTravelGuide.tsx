...
export const MainPartForTravelGuide = () => {
  ...
  useGSAP(
    () => {
      ...
      tl.fromTo(".logo-logo-travel", { opacity: 0 }, { opacity: 1, duration: 1 }, 0.1);
      tl.fromTo(
        ".nav-item-travel",
        { opacity: 0 },
      ...
      tl.to(".reservation-bg-simulation-travel", { height: 0, duration: 0.8 }, 1);

      //   animacion exit para darle paso al contenido principal

      let tl2 = gsap.timeline({
        scrollTrigger: {
          trigger: '.card-grid-first-section', // 👈 Usar ref del contenedor
          start: "top top",
          end: "25% top",
          scrub: true,
  ...
  return (
    <div
      className="main-reusable main-travel absolute overflow-x-hidden min-h-[100dvh] z-50 w-full bg-black/40 backdrop-blur-md "
    >
          ...
          <ul className="md:flex hidden flex-col font-body items-center justify-between w-full py-4 gap-16 text-white text-sm">
            {routes.map((route, index) => (
              <li
                ...
                key={route.path}
                className="nav-item-travel w-full relative"
              >
                <div
                ...
                route.path.startsWith("https://wa.me/") ? (
                  ...
                  <Link
                    to={route.path}
                    className={`block text-pretty font-roboto font-light py-3 px-4 mx-auto ${
...
export default MainPartForTravelGuide;
